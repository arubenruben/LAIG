...
uniform float SCLinesHeight;
uniform float SCLinesRate;
uniform float SCRadialGradient;
...
void main(){
    ...
    float value = 1.0 - (distance * SCRadialGradient);
    ...
    // Makes the white lines that are moving one the security camera
    if(mod(vTextureCoord.y * SCLinesHeight - timeFactor / SCLinesRate, 2.0) > 1.0)
        color = vec4(color.rgb + 0.3,1.0);
    gl_FragColor = vec4(color.rgb * value, 1.0);
